...
export class Logger {
	...
	protected logsStreams: Map<string, fs.WriteStream> = new Map<string, fs.WriteStream>();
	...
	constructor (logsDirFullPath: string, documentRoot: string) {
		logsDirFullPath = pathUtil.resolve(logsDirFullPath).replace(/\\/g, '/');
		documentRoot = pathUtil.resolve(documentRoot).replace(/\\/g, '/');
		this.logsDirFullPath = Helpers.TrimRight(logsDirFullPath, '/');
		this.documentRoot = Helpers.TrimRight(documentRoot, '/');
		// allow all levels by default:
		for (var levelName in Logger.LEVEL) 
			this.allowedLevels.set(Logger.LEVEL[levelName], true);
		process.on('beforeExit', (code) => {
			this.logsStreams.forEach((stream: fs.WriteStream, level: string) => {
				try {
					stream.end();
					stream.close();
				} catch (e) {}
	...
	public SetMaxLogFileSize (maxBytes: number | string = '50M'): Logger {
		if (!isNaN(maxBytes as any)) {
			...
			if (multiplier.length > 0) {
				...
				var multipliers: Map<string, number> = Helpers.ObjectToMap({
					K: 1024,
					M: 1048576,
					G: 1073741824,
					T: 1099511627776
	...
	public Error (err: Error | string, level: string = 'error'): Logger {
		...
		if (err instanceof Error) {
			errType = Helpers.RealTypeOf(err);
		...
		if (this.streamWriting) {
			...
			if (this.logsStreamsLengths.has(level) && this.logsStreamsLengths.get(level) > this.maxLogFileSize) {
				if (!this.logsStreams.has(level)) {
					...
					var stream: fs.WriteStream = this.logsStreams.get(level);
					stream.end();
					stream.close();
			...
			fs.exists(logFullPath, (exists: boolean) => {
				if (!exists) {
					this.appendToLogFileByStandardWrite(msg, level, logFullPath);
				} else {
					fs.stat(logFullPath, (errLocal: Error, stats: fs.Stats) => {
						if (errLocal) {
							this.logsCaches.set(level, this.logsCaches.get(level) + msg);
	...
	protected renameFullLogFile (level: string, cb: () => void) {
		...
		fs.stat(oldFullPath, (errLocal1: Error, stats: fs.Stats) => {
			if (errLocal1) 
			...
			fs.readdir(this.logsDirFullPath, (errLocal2: Error, files: string[]) => {
				if (errLocal2) 
	...
	protected serializeWhatIsPossible (obj: any, prettyPrint: boolean = false, addTypeName: boolean = true): string {
		var result: string[] = [],
			...
			keys: string[],
			key: string;
		if (Helpers.IsPrimitiveType(obj)) {
			...
			for (var i: number = 0, l = obj.length; i < l; i++) {
				...
				try {
					...
					try {
						result.push(v8.serialize(obj[i]).toString('base64'));
					} catch (e2) {
						result.push(JSON.stringify('[' + Helpers.RealTypeOf(obj[i]) + ']'));
					}
				}
				separator = ',';
			}
			...
			for (var j: number = 0, k: number = keys.length; j < k; j++) {
				...
				try {
					...
					try {
						result.push(v8.serialize(obj[key]).toString('base64'));
					} catch (e2) {
						result.push(JSON.stringify('[' + Helpers.RealTypeOf(obj[key]) + ']'));
					}
				}
				separator = ',';
			}
		...
		if (addTypeName)
			result.push(' [' + Helpers.RealTypeOf(obj) + ']');
		return result.join('');
	}
